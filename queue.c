...
pthread_mutex_t queue_lock;	//lock sulla coda
pthread_cond_t empty;		//var. cond. coda vuota
pthread_cond_t full;		//var. cond. coda piena
int queue_running=0;	//variabile condivisa. Indica se MasterWorker continua ad inserire task in coda

//crea la coda
char **create_queue(size_t size) {
	dim=size;
	ec_null(queue=malloc(dim*sizeof(char*)),"queue, s.c. creazione coda");
	pthread_mutex_init(&lock,NULL);	//inizializza il lock della coda
	int i;
...
int enqueue(const char *filename, char** q) {
